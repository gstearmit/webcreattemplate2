...
class GuestsController extends AppController {
	...
	function index() {
		  $this->account();		
	      $this->set('Guest', $this->Guest->read(null,1));
	}
	//Them bai viet
	function add() {
		$this->account();
		if (!empty($this->data)) {
			...
			$data['Guest']['images']=$_POST['userfile'];
			if ($this->Guest->save($data['Guest'])) {
	...
	function view($id = null) {
		...
		$this->set('views', $this->Guest->read(null, $id));
	}
	//close tin tuc
	...
	function edit($id = null) {
		...
		$this->set('edit',$this->Guest->findById($id));
	}
	// Xoa cac dang
