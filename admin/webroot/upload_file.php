...
class FileUploader {
 ...
 // script để thực hiện quá trình upload
	...
	function __construct($fileTypesAllowed = ' pdf, doc, docx ', $fileSizeMax = 100, $uploadDir = 'files/pdf/') {
	...
	*/
	function showFormUpload() {
		...
		<form action="'.htmlentities($_SERVER['PHP_SELF']).'" method="post" enctype="multipart/form-data" onsubmit="wait()">
			...
			<input type="submit" name="submit" value="Up file">
		</form>
		<div id="wait"></div>
	...
	*/
	function getLink($i) {
		...
		$return .= "<tr><td colspan=\"2\"><a href=\"javascript:window.opener.document.image.file.value='$link_bs';window.close();window.history.go(1);\">close</a></td> </tr></table>";
		return $return;
	}
}
?>
