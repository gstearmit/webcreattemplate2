<?php
class LuudiepController extends AppController {
	var $name = 'Luudiep';
	var $uses = array (
			'Product',
			'Tems',
	...
	function chi_tiet_san_pham($id = null) {
		...
		$this->set ( 'list_others', $this->Productshop->find ( 'all', array (
				'conditions' => array (
						'Productshop.status' => 1,
						'Productshop.categoryshop_id' => $catproduct_id,
						'Productshop.id <>' => $id,
						'Productshop.shop_id' => $user 
				),
				'limit' => 10 
		) ) );
		$this->set ( 'title_for_layout', 'Chi tiết sản phẩm' );
	...
	function addshopingcart($id = null) {
		...
		/*
		 * Neu co ma san pham $shopingcart[$id]['name']=$product['Product']['title'];
		 */
		...
		if (isset ( $_SESSION ['shopingcart'] )) {
			...
			if (isset ( $shopingcart [$id] )) {
				echo '<script language="javascript"> if(!confirm("Product exits !. you want continue ?")) window.back(); </script>';
				...
				echo '<script language="javascript"> alert("Product + 1"); window.location.replace("' . DOMAIN . 'home"); </script>'; // co the thay DOMAIN bang url ban muon chay toi
			} else {
				// viet nam
				$shopingcart [$id] ['name'] = $product ['Product'] ['title'];
				$shopingcart [$id] ['images'] = DOMAINAD . $product ['Product'] ['images'];
				$shopingcart [$id] ['sl'] = 1;
				$shopingcart [$id] ['price'] = $product ['Product'] ['price'];
				$shopingcart [$id] ['introduction'] = $product ['Product'] ['introduction'];
				$shopingcart [$id] ['content'] = $product ['Product'] ['content'];
				$shopingcart [$id] ['display'] = $product ['Product'] ['display'];
				$shopingcart [$id] ['giacu'] = $product ['Product'] ['giacu'];
				$shopingcart [$id] ['giamgia'] = $product ['Product'] ['giamgia'];
				$shopingcart [$id] ['total'] = $shopingcart [$id] ['price'];
				$shopingcart [$id] ['size'] = $_SESSION ['size'];
				$shopingcart [$id] ['cate'] = $_SESSION ['cate'];
				$_SESSION ['shopingcart'] = $shopingcart;
				$this->set ( compact ( 'shopingcart' ) );
				echo '<script language="javascript"> alert("Success!"); window.location.replace("' . DOMAIN . 'home"); </script>';
			}
		}
	}
	...
	function buy() {
		if (isset ( $_SESSION ['shopingcart'] )) {
			...
			echo '<script language="javascript"> alert("Product emty !"); window.location.replace("' . DOMAIN . 'home"); </script>';
		}
	}
}
