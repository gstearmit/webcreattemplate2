<?php
class CategoryshopController extends AppController {

	var $name = 'Categoryshop';
	var $uses=array('Categoryshop','Shop','User','Userscms');
	...
	function index() {	
	 $this->checkIfLogged();
	  ...
	  foreach($temshop as $temshop){
		...
		$category = $this->Categoryshop->find('all',array('conditions'=>array('Categoryshop.status'=>1,'Categoryshop.shop_id'=>$temshop['Shop']['id']),'order'=>'Categoryshop.id DESC'));

	   $this->set('categoryshop',$category);
	   $categoryshop = $temshop['Shop']['id'];
	 
	 
	  //$list_cat = $this->Categoryshop->generatetreelist('Categoryshop.shop_id = "'.$categoryshop.'"',null,null,"--- ");
	...
	function add(){ 
	  $this->checkIfLogged();
		...
		if (!empty($this->data)) 
		{
			...
			foreach ($temshop as $temshop){
			$this->data['Categoryshop']['shop_id']=$temshop['Shop']['id'];
			break;
			}
			if ($this->Categoryshop->save($this->data)) {
