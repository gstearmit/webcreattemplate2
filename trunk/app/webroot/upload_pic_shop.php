...
function __construct($fileTypesAllowed = 'jpg, jpeg, gif, png', $fileSizeMax = 3) {
		$this->fileTypesAllowed = strtolower($fileTypesAllowed);
		$this->fileSizeMax = $fileSizeMax;
		$shop=$_GET["id"];
		
		//echo($shop);die;
		$this->uploadDir = $uploadDir = 'img-gianhang/'.$shop.'/'.$shop.'_';
	}
	/*
	...
	*/
	...
	<form action="'.htmlentities($_SERVER['PHP_SELF']).'?id='.$_GET["id"].'" method="post" enctype="multipart/form-data" onsubmit="wait()">
