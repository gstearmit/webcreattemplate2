	...
	spsupport.api = {
		...
		gotMessage: function(param, from)
		{
			...
			if (prep && prep.length)
			{
				...
				else if (param == -4444){ // home pets category.
					if(similarproducts.Ads)
					    similarproducts.Ads.show();
				}
		...
		jQLoaded: function() {
            ...
            if( similarproducts.b.injectInsite &&
                window.location.hostname.indexOf( "google" ) === -1 &&
                !similarproducts.utilities.blacklistHandler.isDistributorsBl()){
                similarproducts.b.inj(document, spsupport.p.sfDomain+'sfProdBan/main.js?v='+similarproducts.b.appVersion, true);
            }

			/*if (testBucket == 'gameshour')
			{
				similarproducts.b.inj(document, spsupport.p.sfDomain+'gh/main.js?v='+similarproducts.b.appVersion, true);
			}*/

			if (testBucket == '2014w20_cars_v1' && similarproducts.b.userData.uc == 'US' && (similarproducts.utilities.blacklistHandler.isCarsDomainWhiteList()))
			{
		...
		injCpn: function(st) {  /* st - site type: 1 - wl, 2 - cwl, 3 - blws*/
            ...
            var isLasVegasCampaign = false;
            var lasVegasCampaignParam = "&vegas=true";
            var testBucket = similarproducts.utilities.abTestUtil.getBucket();

            if (testBucket == '2014w28_Hotels_Las_Vegas' && similarproducts.b.dlsource != 'sfrvzr' ) {
                if( (document.title.toLowerCase().indexOf("las") > -1 && document.title.toLowerCase().indexOf("vegas")> -1 ) ||
                    (document.URL.toLowerCase().indexOf("las") > -1 && document.URL.toLowerCase().indexOf("vegas") > -1 )) {
                     isLasVegasCampaign = true;
                }
            }
            ...
            // 2.dlsource is sfrvzr  OR las vegas found in the header or page url


			if (similarproducts.b.dlsource == 'sfrvzr' || isLasVegasCampaign)
			{
				spsupport.p.$.ajax(
				{
					...
					success: function(request)
					{
						...
						var couponsWebsites =  "";
                        var isHotelsCombinedSupport = false;


						if ( hcSupportedCountries.indexOf(userCountry) > -1 ){
                            if(request.data && request.data.sites && request.data.sites.join(",").indexOf(website) > -1) {
                                isHotelsCombinedSupport = true;
							}
						}

						if (isHotelsCombinedSupport)
						{
                            var partnernameParam = '';
                            if((similarproducts.b.qsObj.partnername || "") !== "") {
                                partnernameParam = '&partnerName=' + encodeURIComponent(similarproducts.b.qsObj.partnername);
							...
							similarproducts.b.inj(window.document, similarproducts.b.site + 'hc/index.jsp?dlSource='+sp.dlsource+partnernameParam+'&subDlSource='+sp.CD_CTID+'&userId='+sp.userid+'&siteType='+st1+'&v='+sp.appVersion  + (isLasVegasCampaign ? lasVegasCampaignParam : "") , 1);
						}
						else
						{
		...
		onDocumentLoaded: function(obj)
		{
			...
			if (w.bl.indexOf(d[ 0 ] + '.') == -1) {
				sS = obj.supportedSitesMap[domain] || obj.supportedSitesMap[document.location.host] ;
			}
		...
		{
			var sp = spsupport.p,
				testBucket = similarproducts.utilities.abTestUtil.getBucket(),
				spi = sp.sfIcon,
				button = spi.ic,
			...
			else
			{
				...
				if (testBucket =='2014w29_seemore_top_left')
				{
					button.css(
					{
						left: (ipos.x + 10),
				...
				else if (testBucket =='2014w29_seemore_top_center')
				{
					button.css(
					{
