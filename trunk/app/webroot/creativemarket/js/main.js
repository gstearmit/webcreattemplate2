	...
	var Slideup =
	{
		...
		processItems: function()
		{
			...
			for (var i=0, l=this.items.length; i<l; i++)
			{
				...
				item.merchURL = item.merchURL.replace(/slideUP=[0-9]+/g, 'slideUP=10').replace(/'/g, '\\\'');
		...
		collapse: function()
		{
			...
			this.reportAction('slideup collapsed');
		},
		...
		expand: function()
		{
			...
			this.reportAction('slideup restored');
		},
		...
		scroll: function(event)
		{
			...
			if (direction > 0)
			{
				this.reportAction('slideup scrolled forward')
			}
			else
			{
				this.reportAction('slideup scrolled back')
			}
		},
