...
function winOnScroll() {
  ...
  xSlideTo('floating_banner_left', (screen.width - (1100-775) - 770)/2-90 , y, slideTime);  // Chỉnh khoảng cách bên trái
  xSlideTo('floating_banner_right', (screen.width - (350-795) +595)/2-24, y, slideTime); // // Chỉnh khoảng cách bên Phải
}
...
<div id="floating_banner_left_content">
 <?php $advleft = $this -> requestAction('/'.$shopname.'/advleft');?>
     <?php foreach($advleft as $advlefts){?>
        <a href="" target="_blank" ><img src="<?php echo DOMAINADBUSINISS;?><?php echo $advlefts['Eshopdaquyadvertisement']['images'];?>" border="0" width="120px" height="" alt="" /></a><br />
     <?php }?>
</div>
</div>
<div id="floating_banner_right" style="text-align:left; position:absolute; overflow:hidden; margin-top:15px !important; right: 0px; width: 125px; border: 0px solid #000;">
    ...
    <?php $advright = $this -> requestAction('/'.$shopname.'/advright');?>
     <?php foreach($advright as $advrights){?>
        <a href="" target="_blank" ><img src="<?php echo DOMAINADBUSINISS;?><?php echo $advrights['Eshopdaquyadvertisement']['images'];?>" border="0" width="120px" height="" alt="" /></a>
     <?php }?>
</div>
</div>
<script>
