 ...
 // script để thực hiện quá trình upload
	...
	*/
	function showFormUpload() {
		...
		<form action="'.htmlentities($_SERVER['PHP_SELF']).'?stt='.$_GET['stt'].'" method="post" enctype="multipart/form-data" onsubmit="wait()">
	...
	*/
	function getLink($i) {
		...
		$return .= "<tr><td colspan=\"2\"><a href=\"javascript:window.opener.document.image.userfile".$_GET['stt'].".value='$link_bs';window.close();window.history.go(1);\">close</a></td> </tr></table>";
		return $return;
	}
}
?>
